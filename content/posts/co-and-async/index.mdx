...
const async: Async = (genFn) =>
  (...args) => new Promise((resolve, reject) => {
    ...
    function(next: () => IteratorResult<unknown>) {
      let result = nextF()
...
同时我们要在 next 内部执行 gen.next，通过包裹一个函数传入，在内部得到 result
...
现在我们加上错误处理，当 resolve value 出错时会通过 gen.throw(err) 抛出错误，而 gen.throw 通过 genF 内部的 try / catch 捕捉，如果内部没有捕捉到再通过上面的 try / catch 捕捉

</CodeWave>

现在我们完成和 async / await 的函数的模拟，我们看到 async function 实际上返回一个 Promise 包裹的 return 值，await 后会自动使用 Promise.resolve 进行包裹
